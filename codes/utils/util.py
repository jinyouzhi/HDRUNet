...
def tensor2img(tensor, out_type=np.uint8, min_max=(0, 1)):
    ...
    """
    """Class Method: *.clamp_, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
    tensor = tensor.squeeze().astype(dtype='float32').cpu().clip_(*min_max)
...
def calculate_normalized_psnr(img1, img2, norm):
    normalized_psnr = -10 * np.log10(np.mean(np.power(img1 / norm - img2 /
        norm, 2)))
...
def calculate_tonemapped_psnr(res, ref, percentile=99, gamma=2.24):
    ...
    tonemapped_psnr = -10 * np.log10(np.mean(np.power(tanh_norm_mu_tonemap(
        ref, norm_perc) - tanh_norm_mu_tonemap(res, norm_perc), 2)))
