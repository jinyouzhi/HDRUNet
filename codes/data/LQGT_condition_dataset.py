...
class LQGT_dataset(paddle.io.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.sizes_LQ, self.paths_LQ = util.get_image_paths(self.data_type,
            opt['dataroot_LQ'])
        self.sizes_GT, self.paths_GT = util.get_image_paths(self.data_type,
            opt['dataroot_GT'])
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            if GT_size != 0:
                ...
                img_GT = img_GT[rnd_h_GT:rnd_h_GT + GT_size, rnd_w_GT:
                    rnd_w_GT + GT_size, :]
            img_LQ, img_GT = util.augment([img_LQ, img_GT], self.opt[
                'use_flip'], self.opt['use_rot'])
        ...
        if img_GT.shape[2] == 3:
            ...
            cond = cond[:, :, [2, 1, 0]]
        H, W, _ = img_LQ.shape
        img_GT = paddle.to_tensor(data=np.ascontiguousarray(np.transpose(
            img_GT, (2, 0, 1)))).astype(dtype='float32')
        ...
        if LQ_path is None:
            LQ_path = GT_path
        return {'LQ': img_LQ, 'GT': img_GT, 'cond': cond, 'LQ_path':
            LQ_path, 'GT_path': GT_path}
